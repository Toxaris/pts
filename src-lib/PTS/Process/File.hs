...
filterRet name (cache, imports, bindings) = result where
  result = Module imports name contents
  contents = Map.mapMaybe transform bindings

  transform (True, t, v) = Just (False, t, v)
  transform _ = Nothing
...
processStmt (Bind n args Nothing body) = recover () $ do
  ...
  modify $ (\f (x, y, z) -> (x, y, f z)) $ Map.insert n (False, v, q)
...
processStmt (Export n) = recover () $ do
  ...
  -- mark as exported
  (cache, imports, bindings) <- get
  when (n `Map.notMember` bindings) $ do
    fail $ "Unbound identifier: " ++ show n
  let bindings' = Map.adjust (\(x, t, v) -> (True, t, v)) n bindings
